...
module Vessel
  ...
  def enter target
    ...
    setParent(target.id) ; save

    return "You entered #{target.print}."
  ...
  def leave target
    ...
    return "You left the current vessel, and entered #{target.print}."
  ...
  def warp target
    ...
    return "You warped to #{target.print}."
  ...
  def look params = nil
    ...
    if parent == id
      ...
      text += "~ "+"#{print} in #{$parade.parent.print}.\n\n".capitalize
    end

    # NOTE
    note = $parade.parent.note.to_s
    $parade.visible.each do |vessel|
      if note.include?(vessel.name)
        note = note.sub(vessel.name,"#{vessel.name}")
        used.push(vessel.name)
      end
    end
    ...
    # PROGRAM
    text += $parade.parent.program.isValid ? "#{$parade.parent.program.debug}\n" : ""

    text += "\n\n"

    $parade.visible.each do |vessel|
      used.push(vessel)
      text += vessel.display_for(self,$parade.inventory)
    end
    text += "\n\n"

    # HINT
    text += $parade.parent.hint ? "? #{$parade.parent.hint}" : ""
  ...
  def connect params = nil
    ...
    size = (($parade.world.length/20000.0)*1000).to_i / 10.0

    prompt  = "; #{print} connected on Septamber 45.\n"
    prompt += "; ESTATE #{size}% VESSELS #{$parade.world.length}\n"
    prompt += "; Type help for more info, or explore."
    prompt += "\n\n\n"
    return prompt

  end

  def use params = nil # Run program on self

    target = $parade.vesselWithName($parade.visible,params)
    puts "Okay, player"
    return target._use

  end

  def _use # Fires when used

    return program.run(@player,@inventory)

  end

  def operate params = nil # Run program on vessel
